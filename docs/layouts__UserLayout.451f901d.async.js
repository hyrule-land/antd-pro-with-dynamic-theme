(window['webpackJsonp'] = window['webpackJsonp'] || []).push([
  [3],
  {
    '2n1B': function(I, M, g) {
      'use strict';
      var e = g('g09b');
      Object.defineProperty(M, '__esModule', { value: !0 }), (M.default = void 0), g('Pwec');
      var N = e(g('CtXQ'));
      g('lUTK');
      var A = e(g('BvKs')),
        D = g('Y2fQ'),
        t = e(g('q1tI')),
        C = e(g('TSYQ')),
        a = e(g('6Wvd')),
        T = e(g('Kkfi')),
        j = function(I) {
          var M = I.className,
            g = (0, D.getLocale)(),
            e = function(I) {
              var M = I.key;
              return (0, D.setLocale)(M);
            },
            j = ['zh-CN', 'zh-TW', 'en-US', 'pt-BR'],
            u = {
              'zh-CN': '\u7b80\u4f53\u4e2d\u6587',
              'zh-TW': '\u7e41\u4f53\u4e2d\u6587',
              'en-US': 'English',
              'pt-BR': 'Portugu\xeas',
            },
            l = {
              'zh-CN': '\ud83c\udde8\ud83c\uddf3',
              'zh-TW': '\ud83c\udded\ud83c\uddf0',
              'en-US': '\ud83c\uddfa\ud83c\uddf8',
              'pt-BR': '\ud83c\udde7\ud83c\uddf7',
            },
            c = t.default.createElement(
              A.default,
              { className: T.default.menu, selectedKeys: [g], onClick: e },
              j.map(function(I) {
                return t.default.createElement(
                  A.default.Item,
                  { key: I },
                  t.default.createElement('span', { role: 'img', 'aria-label': u[I] }, l[I]),
                  ' ',
                  u[I],
                );
              }),
            );
          return t.default.createElement(
            a.default,
            { overlay: c, placement: 'bottomRight' },
            t.default.createElement(
              'span',
              { className: (0, C.default)(T.default.dropDown, M) },
              t.default.createElement(N.default, {
                type: 'global',
                title: (0, D.formatMessage)({ id: 'navBar.lang' }),
              }),
            ),
          );
        },
        u = j;
      M.default = u;
    },
    '6Wvd': function(I, M, g) {
      'use strict';
      var e = g('g09b');
      Object.defineProperty(M, '__esModule', { value: !0 }), (M.default = void 0), g('qVdP');
      var N = e(g('jsC+')),
        A = e(g('jehZ')),
        D = e(g('Y/ft')),
        t = e(g('q1tI')),
        C = e(g('TSYQ')),
        a = e(g('QyDn')),
        T = function(I) {
          var M = I.overlayClassName,
            g = (0, D.default)(I, ['overlayClassName']);
          return t.default.createElement(
            N.default,
            (0, A.default)({ overlayClassName: (0, C.default)(a.default.container, M) }, g),
          );
        },
        j = T;
      M.default = j;
    },
    BOD2: function(I, M, g) {
      I.exports = {
        container: 'antd-pro-layouts-user-layout-container',
        lang: 'antd-pro-layouts-user-layout-lang',
        content: 'antd-pro-layouts-user-layout-content',
        top: 'antd-pro-layouts-user-layout-top',
        header: 'antd-pro-layouts-user-layout-header',
        logo: 'antd-pro-layouts-user-layout-logo',
        title: 'antd-pro-layouts-user-layout-title',
        desc: 'antd-pro-layouts-user-layout-desc',
      };
    },
    Kkfi: function(I, M, g) {
      I.exports = {
        menu: 'antd-pro-components-select-lang-index-menu',
        dropDown: 'antd-pro-components-select-lang-index-dropDown',
      };
    },
    QyDn: function(I, M, g) {
      I.exports = { container: 'antd-pro-components-header-dropdown-index-container' };
    },
    mxmt: function(I, M) {
      I.exports =
        'data:image/svg+xml;base64,';
    },
    'nsf+': function(I, M, g) {
      'use strict';
      var e = g('g09b');
      Object.defineProperty(M, '__esModule', { value: !0 }), (M.default = void 0);
      var N = e(g('p0pE')),
        A = g('Hx5s'),
        D = g('TJpk'),
        t = e(g('wY1l')),
        C = e(g('q1tI')),
        a = g('MuoO'),
        T = g('Y2fQ'),
        j = e(g('2n1B')),
        u = e(g('mxmt')),
        l = e(g('BOD2')),
        c = function(I) {
          var M = I.route,
            g = void 0 === M ? { routes: [] } : M,
            e = g.routes,
            a = void 0 === e ? [] : e,
            c = I.children,
            z = I.location,
            n = void 0 === z ? { pathname: '' } : z,
            w = (0, A.getMenuData)(a),
            i = w.breadcrumb,
            d = (0, A.getPageTitle)(
              (0, N.default)(
                { pathname: n.pathname, breadcrumb: i, formatMessage: T.formatMessage },
                I,
              ),
            );
          return C.default.createElement(
            C.default.Fragment,
            null,
            C.default.createElement(
              D.Helmet,
              null,
              C.default.createElement('title', null, d),
              C.default.createElement('meta', { name: 'description', content: d }),
            ),
            C.default.createElement(
              'div',
              { className: l.default.container },
              C.default.createElement(
                'div',
                { className: l.default.lang },
                C.default.createElement(j.default, null),
              ),
              C.default.createElement(
                'div',
                { className: l.default.content },
                C.default.createElement(
                  'div',
                  { className: l.default.top },
                  C.default.createElement(
                    'div',
                    { className: l.default.header },
                    C.default.createElement(
                      t.default,
                      { to: '/' },
                      C.default.createElement('img', {
                        alt: 'logo',
                        className: l.default.logo,
                        src: u.default,
                      }),
                      C.default.createElement('span', { className: l.default.title }, 'Ant Design'),
                    ),
                  ),
                  C.default.createElement(
                    'div',
                    { className: l.default.desc },
                    'Ant Design \u662f\u897f\u6e56\u533a\u6700\u5177\u5f71\u54cd\u529b\u7684 Web \u8bbe\u8ba1\u89c4\u8303',
                  ),
                ),
                c,
              ),
              C.default.createElement(A.DefaultFooter, null),
            ),
          );
        },
        z = (0, a.connect)(function(I) {
          var M = I.settings;
          return (0, N.default)({}, M);
        })(c);
      M.default = z;
    },
  },
]);
